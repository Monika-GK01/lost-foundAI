              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                <div className="h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                  {match.foundItem.images?.[0] ? (
                    <img src={match.foundItem.images[0]} alt={match.foundItem.title} className="h-full w-full object-cover" />
                  ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <span className={`inline-block rounded-full px-3 py-1 text-sm font-bold ${
                        match.scores.overallScore >= 70 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                        match.scores.overallScore >= 40 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' :
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {match.scores.explanation?.map((exp, i) => (
                      <span key={i} className="rounded-md bg-gray-100 px-2 py-0.5 text-xs text-[var(--color-text-secondary)] dark:bg-gray-800">{exp}</span>
                    ))}
                  </div>
